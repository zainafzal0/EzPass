...
function CardsPage() {
  ...
  useEffect(() => {    
    ...
    setLastIndex(10);
    Axios.get(
      `http://localhost:3001/api/getCards?user_id=${loginInfo.id}`
    ).then((response) => {
  ...
  return (
    <div>
      ...
      <AddCardModal setSendRequest={setSendRequest} setDefaultImg={setDefaultImg}/>

      <CardModal data={storedCards} storedCards={storedCards} setStoredCards={setStoredCards} modalIndex={infoModaIndex} modalRequest={modalRequest} setModalRequest={setModalRequest}/>
