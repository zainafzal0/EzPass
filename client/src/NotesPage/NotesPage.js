...
function NotesPage() {
  ...
  useEffect(() => {    
    ...
    setLastIndex(10);
    Axios.get(
      `http://localhost:3001/api/getNotes?user_id=${loginInfo.id}`
    ).then((response) => {
  ...
  return (
    <div>
      ...
      <AddNoteModal setSendRequest={setSendRequest} setDefaultImg={setDefaultImg}/>

      <NoteModal data={storedNotes} storedNotes={storedNotes} setStoredNotes={setStoredNotes} modalIndex={infoModaIndex} modalRequest={modalRequest} setModalRequest={setModalRequest}/>
